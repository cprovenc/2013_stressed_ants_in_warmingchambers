...
#Calculating the delta ct method
```{r calculating the delta ct method}

gxp.dat<-(warm[,15:17]-warm[,18])
names(gxp.dat)<-c("xp_40","xp_70","xp_83")
warm2<-as.data.frame(cbind(warm,gxp.dat))
warm2$baittemp.ave<-apply(warm2[,8:11],1,mean,na.rm=TRUE)
#rowMeans(warm[,8:11],na.rm=TRUE)
```

#stats for delta ct method
```{r stats for delta ct method}
#subset(warm2,warm2$xp_70>
#hsp70.mod<-lm(CT_70~baittemp.ave*Site+RIN_Value+CT_actin,data=warm2)
hsp70.mod<-lm(CT_70~baittemp.ave*Site+RIN_Value+CT_18s,data=warm2)
summary(hsp70.mod)
summary(stepAIC(hsp70.mod,direction="backward"))


hsp83.mod<-lm(CT_83~baittemp.ave*Site+RIN_Value+CT_18s,data=warm2)
summary(hsp83.mod)
summary(stepAIC(hsp83.mod,direction="backward"))

#hsp40

hsp40.mod<-lm(CT_40~baittemp.ave*Site+RIN_Value+CT_18s,data=warm2)
summary(hsp40.mod)
summary(stepAIC(hsp40.mod,direction="backward"))
...
ggplot(warm2,aes(x=baittemp.ave,y=CT_83,colour=factor(Site)))+geom_point()
...
#Statistics    
### Predictor: site (HF or DF) and ground temperature (degrees Celsius)
### Response: HSP40, HSP70, and HSP83 gene expression
